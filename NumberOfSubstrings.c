...
void output(char **patterns, int patternsSize, char *word) {
    printf("In a word \"%s\" appear %d substrings from array { ", word, numOfStrings(patterns, patternsSize, word));
    for(int i = 0; i < patternsSize; i++) {
        printf("\"%s\" ", patterns[i]);
    }
    printf("}\n");
