...
void output(int *candies, int candiesSize, int extraCandies, int* returnSize) {
    bool *results = kidsWithCandies(candies, candiesSize, extraCandies, returnSize);
    printf("If you give %d extra candies to\n", extraCandies);
    for(int i = 0; i < candiesSize; i++) {
        printf("Kid %d (%d candies): they'll have %d candies, which is ", i + 1, candies[i], candies[i] + extraCandies);
        if(!results[i]) {
            printf("not ");
        }
